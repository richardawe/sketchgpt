...
| **A worked example teaches its shape, not its rule** | With one example — two objects and a ground line — Qwen3-0.6B asked for "a house" drew **two houses and a ground line**. It copied the example's command count and filled both object slots with the only noun it had. One example is a template; **two examples of different lengths** are a rule. The same applies to the title: the example's title has to show the imperative being stripped, or the model echoes "Draw a house" back as the caption. |
...
| **A chat history of drawings is unbounded and does not need to be** | Sketch history grew by a whole drawing per turn. Carrying only the previous drawing and the instruction that produced it makes the prompt **O(1) in turns** — measured flat at 374 tokens from turn 2 onward at 4096, 2048 and 1024 context. A revision needs a seed, not a transcript. |
