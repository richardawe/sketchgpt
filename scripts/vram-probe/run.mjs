...
const [model, wasm, ctx, gen, sw] = process.argv.slice(2);
...
const url = `http://127.0.0.1:8099/measure.html?model=${model}&wasm=${wasm}` + (ctx ? `&ctx=${ctx}` : '') + (gen === '1' ? '&gen=1' : '') + (sw !== undefined ? `&sw=${sw}` : '');
await p.goto(url);
